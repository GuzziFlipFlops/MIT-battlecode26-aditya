...
public class CommArray {
    ...
    private static final int CHEESE_LOCATION_START = 44;
    private static final int CHEESE_LOCATION_END = 47;
    ...
    public static void reportCheeseLocation(MapLocation loc) throws GameActionException {
        if (loc == null) return;
        if (Globals.myType != UnitType.RAT_KING) return;
        int packed = packLocation(loc);
        for (int i = CHEESE_LOCATION_START; i <= CHEESE_LOCATION_END; i++) {
            int current = Globals.rc.readSharedArray(i);
            if (current == 0) {
                Globals.rc.writeSharedArray(i, packed);
                return;
        ...
        Globals.rc.writeSharedArray(CHEESE_LOCATION_START, packed);
    }
    ...
    private static void readCheeseLocations() throws GameActionException {
        Globals.numKnownCheeseLocations = 0;
        for (int i = CHEESE_LOCATION_START; i <= CHEESE_LOCATION_END && Globals.numKnownCheeseLocations < 4; i++) {
            MapLocation loc = unpackLocation(Globals.rc.readSharedArray(i));
            if (loc != null) {
                Globals.knownCheeseLocations[Globals.numKnownCheeseLocations++] = loc;
            }
        }
    }
    ...
    public static MapLocation getNearestCheeseLocation() {
        MapLocation nearest = null;
        int minDist = Integer.MAX_VALUE;
        for (int i = 0; i < Globals.numKnownCheeseLocations; i++) {
            MapLocation cheeseLoc = Globals.knownCheeseLocations[i];
            if (cheeseLoc != null) {
                int dist = Globals.myLoc.distanceSquaredTo(cheeseLoc);
                if (dist < minDist) {
                    minDist = dist;
                    nearest = cheeseLoc;
                }
