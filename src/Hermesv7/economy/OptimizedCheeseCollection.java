...
public class OptimizedCheeseCollection {
    ...
    private static boolean findAndCollectNearestCheese() throws GameActionException {
        ...
        MapLocation nearestCheese = findNearestCheeseLocation();
        if (nearestCheese != null && Globals.rc.isMovementReady()) {
    ...
    private static MapLocation findNearestCheeseLocation() {
        ...
        // Check shared array cheese locations
        MapLocation arrayCheeseloc = CommArray.getNearestCheeseLocation();
        if (arrayCheeseloc != null) {
            int dist = Globals.myLoc.distanceSquaredTo(arrayCheeseloc);
            if (dist < minDist) {
                minDist = dist;
                nearest = arrayCheeseloc;
            }
        }
