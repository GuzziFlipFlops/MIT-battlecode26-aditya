...
public class Thrower {
    ...
    public static boolean tryThrowFarAway() throws GameActionException {
        RobotInfo carrying = Globals.rc.getCarrying();
        if (carrying == null) return false;
        if (carrying.getTeam() == Globals.myTeam) return false;
        ...
        MapLocation farAway = new MapLocation(oppositeX, oppositeY);
        
        Direction toFarAway = myLoc.directionTo(farAway);
        if (toFarAway == Direction.CENTER) {
            toFarAway = Direction.NORTH;
        }
        
        if (Globals.myDir != toFarAway && Globals.rc.canTurn()) {
            Globals.rc.turn(toFarAway);
        }
        if (Globals.rc.canThrowRat()) {
            Globals.rc.throwRat();
            if (Globals.roundNum % 20 == 0) {
                System.out.println("[DEBUG Thrower] Rat #" + Globals.myID + " threw enemy rat far away to " + farAway);
            }
            return true;
        }
        return false;
